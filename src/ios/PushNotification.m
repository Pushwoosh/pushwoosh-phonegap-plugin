...
- (void)onDeviceReady:(CDVInvokedUrlCommand *)command {
    // 1) Get the options safely
    id first = command.arguments.count > 0 ? command.arguments[0] : nil;
    NSDictionary *options = ([first isKindOfClass:[NSDictionary class]]) ? (NSDictionary *)first : @{};
    // Useful while debugging:
    NSLog(@"[PushNotification onDeviceReady] options = %@", options);

    // 2) Normalize appid from multiple possible keys, stripping NSNull
    id appIdObj = options[@"pw_appid"] ?: options[@"appid"] ?: options[@"appId"];
    if (appIdObj == (id)[NSNull null]) appIdObj = nil;
    NSString *appid = [appIdObj isKindOfClass:[NSString class]] ? (NSString *)appIdObj : nil;

    NSString *appname = nil;
    id appNameObj = options[@"appname"] ?: options[@"appName"];
    if (appNameObj == (id)[NSNull null]) appNameObj = nil;
    appname = [appNameObj isKindOfClass:[NSString class]] ? (NSString *)appNameObj : nil;

    // 3) Validate and init, then ALWAYS resolve success/fail
    if (appid.length == 0) {
    ...
    [UNUserNotificationCenter currentNotificationCenter].delegate =
        [PushNotificationManager pushManager].notificationCenterDelegate;
    ...
    NSMutableDictionary *payload = [PushNotificationManager getRemoteNotificationStatus];
    payload[@"initialized"] = @YES;
